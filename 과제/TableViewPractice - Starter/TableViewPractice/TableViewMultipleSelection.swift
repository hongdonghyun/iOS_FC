...
final class TableViewMultipleSelection: UIViewController {
    ...
    func randomWithoutDuplication(_ selected: [Int] = []) -> [Int] {
        var returnArray: [Int] = selected
    ...
    @objc func reloadData() {
        ...
        randomData = randomWithoutDuplication(selectedRows)
...
// MARK = UITableViewScrollViewDelegate

extension TableViewMultipleSelection: UITableViewDelegate {
    func tableView(_ tableView: UITableView, willSelectRowAt indexPath: IndexPath) -> IndexPath? {
        return randomData[indexPath.row] <= 7 ? nil : indexPath
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        selectedRows.append(randomData[indexPath.row])
    }
}
